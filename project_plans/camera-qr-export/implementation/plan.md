...
| `QrTransferError` | `sealed interface QrTransferError : DomainError` (ADR/Architecture): `ChunkDecodeFailed`, `IntegrityCheckFailed`, `PayloadTooLarge`, `MarkdownParseFailed`. | Added to `error/DomainError.kt` with `toUiMessage` branches. Originally shipped with two additional variants, `IncompleteTransfer(received, total)` and `TransferCancelled` — both removed post-implementation as dead code after a spec-compliance audit found no principled call site (`ChunkBuffer.reassemble()`'s only production caller already gates on `isComplete()`; cancellation is handled entirely via `CoordinatorEvent.Cancelled`/UI state, never as an `Either` `Left`). |
