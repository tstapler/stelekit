...
- **Resolved (triad review, 2026-09-11)**: the macOS custom URL-scheme handler (`research/build-vs-buy.md` §2) is **not** folded into v1. Rationale: it still requires a signed/notarized `.dmg` for `Desktop.setOpenURIHandler()` to survive Gatekeeper on a fresh install (this repo's jpackage output is ad-hoc-signed only, per `research/build-vs-buy.md`), it only helps the macOS surface (Linux/Windows still get nothing), and it would add a second cold-start mechanism alongside the pending-captures-file poller this plan already builds — two cold-start paths for one v1 is scope creep the hotkey-popup-first sequencing (Pattern Decisions row `v1 OS-surface scope`) exists to avoid. The release-notes caveat (Risk Control, above) is the accepted v1 mitigation; the URL-scheme handler is re-scoped into the same Phase 2 OS-surfaces epic as the Services-menu/Nautilus/registry-handler work (ADR-002), not dropped.
- **Resolved**: `tulskiy/jkeymaster` version is `1.3` — already specified in Task 1.3.1a's dependency line (`com.github.tulskiy:jkeymaster:1.3`, resolved via JitPack, not Maven Central, since `com.github.<user>` coordinates are JitPack's convention). No longer blocks Task 1.3.1a; verify the tag still resolves via JitPack at build time as a normal dependency-resolution check, not a design question.
