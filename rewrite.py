...
with open('kmp/src/jvmMain/kotlin/dev/stapler/stelekit/stats/GraphStatsCollector.kt', 'r') as f:
    content = f.read()

# Replace JournalEntry extraction
content = content.replace("""    private data class FileScan(
        val name: String,
        val blocks: Int,
    ...
    fun collect(graphDir: File): GraphStatsReport {""", """    private data class FileScan(
        val name: String,
        val blocks: Int,
        val blocksWithLinks: Int,
        val outgoing: List<String>,
...
content = content.replace("""        // Scan journals, extract dates
        data class JournalEntry(val scan: FileScan, val date: String?)
        val journals = (journalsDir.listFiles { f -> f.extension == "md" } ?: emptyArray())""", """        // Scan journals, extract dates
        val journals = (journalsDir.listFiles { f -> f.extension == "md" } ?: emptyArray())""")

# Extract pageNameFromFile and scan
content = content.replace("""        fun pageNameFromFile(f: File): String =
...
content = content.replace("""        // Per-page connectivity
        fun connectivity(s: FileScan) = PageConnectivity(s.name, incomingCount[s.name] ?: 0, s.outgoing.size)

        val pageConnectivity = pages.map { connectivity(it) }""", """        val pageConnectivity = pages.map { connectivity(it, incomingCount) }""")

# Extract histogram
content = content.replace("""        // Histograms — bucket anything > 20 into the "20" bin for display
        fun histogram(values: Iterable<Int>): Map<Int, Int> =
            values.groupingBy { it.coerceAtMost(20) }.eachCount().toSortedMap()

        val incomingHistogram = histogram(pageConnectivity.map { it.incomingLinks })""", """        val incomingHistogram = histogram(pageConnectivity.map { it.incomingLinks })""")

# Extract epochDay
content = content.replace("""    private fun daysBetween(a: String, b: String): Int {
...
with open('kmp/src/jvmMain/kotlin/dev/stapler/stelekit/stats/GraphStatsCollector.kt', 'w') as f:
    f.write(content)
