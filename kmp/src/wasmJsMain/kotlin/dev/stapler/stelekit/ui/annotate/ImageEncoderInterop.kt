 ...
 * [ByteArray.toCharPerByteJsString]) and is unpacked by one native JS loop over `charCodeAt` — a
 * single interop call total. This is *not* base64: an earlier version base64-encoded the buffer
 * on the Kotlin side first, but that cost ~3.3s of Kotlin/Wasm stdlib `Base64.encode` time alone
 * for a 3000×2000 image (measured via `ImageEncoderWasmJsTest`) — a one-to-one char mapping needs
 * no bit-repacking/lookup-table work and is dramatically cheaper for a buffer this large.
 */
