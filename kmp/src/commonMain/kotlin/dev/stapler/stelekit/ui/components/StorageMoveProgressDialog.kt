 ...
 * @param onSummaryAcknowledged dismisses the minimal `Summary` acknowledgment.
 * @param onReopenFailedAcknowledged `ReopenFailed`'s sole "OK" action — returns to the graph list.
 */
@Composable
fun StorageMoveProgressDialog(
    ...
    onCancel: () -> Unit,
    onRetry: () -> Unit,
    onSummaryAcknowledged: () -> Unit,
    onReopenFailedAcknowledged: () -> Unit,
) {
    Dialog(
        ...
        Surface(shape = MaterialTheme.shapes.medium, tonalElevation = 6.dp) {
            Column(modifier = Modifier.width(360.dp).padding(20.dp)) {
                ...
                when (state) {
                    ...
                    is StorageMoveUiState.Summary -> SummaryStep(graphName, onSummaryAcknowledged)
                    is StorageMoveUiState.Failed -> FailedStep(state.reason, onRetry, onCancel)
                    is StorageMoveUiState.ReopenFailed -> ReopenFailedStep(graphName, onReopenFailedAcknowledged)
                }
            }
        }
    }
...
@Composable
private fun SummaryStep(graphName: String, onAcknowledged: () -> Unit) {
    Row(
    ...
    Spacer(Modifier.height(16.dp))
    Row(horizontalArrangement = Arrangement.End, modifier = Modifier.fillMaxWidth()) {
        Button(onClick = onAcknowledged) { Text("Done") }
    }
}
 ...
 */
@Composable
private fun ReopenFailedStep(graphName: String, onAcknowledged: () -> Unit) {
    Row(verticalAlignment = Alignment.CenterVertically) {
        Icon(
            imageVector = Icons.Default.Warning,
            contentDescription = null,
    ...
    Row(horizontalArrangement = Arrangement.End, modifier = Modifier.fillMaxWidth()) {
        Button(onClick = onAcknowledged) { Text("OK") }
    }
}
