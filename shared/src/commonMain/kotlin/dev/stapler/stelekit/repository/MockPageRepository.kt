...
class MockPageRepository : PageRepository {
    ...
    private val pages = mutableMapOf<String, dev.stapler.stelekit.repository.Page>()

    override suspend fun findById(id: String): dev.stapler.stelekit.repository.Page? = pages[id]

    override suspend fun findAll(): List<dev.stapler.stelekit.repository.Page> = pages.values.toList()

    override suspend fun findAllPaginated(pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
    ...
    override suspend fun save(entity: dev.stapler.stelekit.repository.Page): dev.stapler.stelekit.repository.Page {
    ...
    override suspend fun saveAll(entities: List<dev.stapler.stelekit.repository.Page>): List<dev.stapler.stelekit.repository.Page> {
        val now = Clock.System.now()
    ...
    override suspend fun delete(entity: dev.stapler.stelekit.repository.Page): Boolean = deleteById(entity.id)

    override suspend fun existsById(id: String): Boolean = pages.containsKey(id)

    override suspend fun count(): Long = pages.size.toLong()

    override suspend fun findByName(name: String): dev.stapler.stelekit.repository.Page? =
        pages.values.find { it.name == name }

    override suspend fun findByOriginalName(originalName: String): dev.stapler.stelekit.repository.Page? =
        pages.values.find { it.originalName == originalName }

    override suspend fun findByNamespace(namespace: List<String>): dev.stapler.stelekit.repository.Page? =
        pages.values.find { it.namespace == namespace }

    override suspend fun existsByName(name: String): Boolean =
        pages.values.any { it.name == name }

    override suspend fun findJournals(pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
    ...
    override suspend fun findJournalByDay(day: Int): dev.stapler.stelekit.repository.Page? =
        pages.values.find { it.journalDay == day }

    override suspend fun findJournalsInRange(startDay: Int, endDay: Int): List<dev.stapler.stelekit.repository.Page> =
        pages.values.filter { it.journalDay != null && it.journalDay in startDay..endDay }
            .sortedBy { it.journalDay }

    override suspend fun findPagesInNamespace(namespace: String, pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
    ...
    override suspend fun findChildPages(parentPageId: String, pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
        // Simplified implementation - return empty for mock
        return Page(emptyList(), 0, false)
    }

    override suspend fun findParentPages(childPageId: String): List<dev.stapler.stelekit.repository.Page> {
        // Simplified implementation - return empty for mock
        return emptyList()
    }

    override suspend fun search(criteria: PageSearchCriteria, pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
    ...
    override suspend fun searchByName(query: String, pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> =
        search(PageSearchCriteria(query = query), pagination)

    override suspend fun searchByProperties(properties: Map<String, Any>, pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
        val filteredPages = pages.values.filter { page ->
    ...
    override suspend fun findPagesWithBlocks(blockIds: List<String>): List<dev.stapler.stelekit.repository.Page> {
        // Mock implementation - in real implementation, this would query block-page relationships
        return emptyList()
    }

    override suspend fun findRecentlyUpdated(pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
    ...
    override suspend fun findRecentlyCreated(pagination: Pagination): Page<dev.stapler.stelekit.repository.Page> {
    ...
    override suspend fun updateProperties(pageId: String, properties: Map<String, Any>): dev.stapler.stelekit.repository.Page? {
    ...
    override suspend fun renamePage(pageId: String, newName: String): dev.stapler.stelekit.repository.Page? {
        return pages[pageId]?.let { page ->
    ...
    override suspend fun updateNamespace(pageId: String, newNamespace: List<String>): dev.stapler.stelekit.repository.Page? {
        return pages[pageId]?.let { page ->
    ...
    override fun observePage(pageId: String): Flow<dev.stapler.stelekit.repository.Page?> =
        flowOf(pages[pageId])

    override fun observePagesByNamespace(namespace: String): Flow<List<dev.stapler.stelekit.repository.Page>> =
        flowOf(emptyList())

    override fun observeRecentlyUpdated(): Flow<List<dev.stapler.stelekit.repository.Page>> =
        flowOf(emptyList())
